##########
#
# Running scGen for the ATRIAL fibroblasts
...
nuclei=sc.read_h5ad(path_out + "fibroblasts_nucleis_filt_V2_AT_LEIDEN_RAW.h5ad")
cells=sc.read_h5ad(path_out + "fibroblasts_cells_filt_V2_AT_LEIDEN_RAW.h5ad")
...
sc.pp.highly_variable_genes(tmp_nuclei, n_top_genes=5000)
...
sc.pp.highly_variable_genes(tmp_cells, n_top_genes=5000)


# Concatenate objects
all=nuclei.concatenate([cells])
all.write(path_out + "ALL_corrected_FB_AT_double_RAW.h5ad")
...
nuclei=sc.pp.subsample(nuclei, n_obs=7000, random_state=random_state, copy=True) 
...
all.obs['cell_type']=all.obs['cell_states'].values
all.obs['batch']=all.obs['cell_source'].values
all_train.obs['cell_type']=all_train.obs['cell_states'].values
...
print("Network made")
network.train(train_data=all_train, n_epochs=50)
...
corrected_adata.write(path_out + "ALL_corrected_FB_AT_source.h5ad")
corrected_adata_train.write(path_out + "ALL_corrected_FB_AT_source.h5ad")
...
network = scgen.VAEArith(x_dimension= all_train.shape[1], model_path= path_out + 'scgen_model/scgen_model_ATV_double')
print("Network made")
network.train(train_data=all_train, n_epochs=30)
print("Network trained")
corrected_adata =  batch_removal_ct5(network, all, batch_key="batch", cell_label_key="cell_type")

# Finished with batch alignment

corrected_adata.write(path_out + "ALL_corrected_FB_AT_double.h5ad")
